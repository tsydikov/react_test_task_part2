...
function App() {
  ...
  const calck = () => {
    ...
    for (let i = startWork[1]; startWork[0] < endWork[0]; i = i + 45) {
      // проверка совещания
      let diference = Math.abs((startWork[0]-appointments[numberOfAppointment][0])* 60 + (appointments[numberOfAppointment][1] - startWork[1]))
      if (diference<= 45) {
        // console.log(diference, startWork,numberOfAppointment)
